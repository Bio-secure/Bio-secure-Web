...
export default defineComponent({
  ...
  setup() {
    ...
    const user = ref({
      ...
      surname: '',
      phone: '',
      id: '', // This will be BirthDate
      balance: 0,
      face_image_url: '' // For the dynamic profile picture
    });

    // We will keep the transaction logic but start with an empty list
    ...
    onMounted(async () => {
      const customerId = route.params.id; // Get customer ID from URL, e.g., /info/12345
      if (!customerId) {
      ...
      try {
        ...
        user.value = {
          ...
          face_image_url: data.face_image_url // Assign the image URL from the backend
        };
    ...
    // Combine name and surname into a single computed property
    const fullName = computed(() => `${user.value.name} ${user.value.surname}`);

    const submitTransaction = () => {
      // This function can remain for UI demonstration purposes
      console.log('Transaction submitted');
    };

    return {
      user,
      fullName, // Return fullName to the template
      transactions,
      transaction,
      isDepositMode,
      isLoading,
        ...
        <table class="w-full text-sm border">
          ...
          <tbody>
            <tr v-if="transactions.length === 0">
              <td colspan="3" class="text-center p-4 text-gray-500">No transaction history found.</td>
            </tr>
