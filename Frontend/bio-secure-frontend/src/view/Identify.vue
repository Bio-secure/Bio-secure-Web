...
onMounted(async () => {
  try {
    ...
    // The displayName is created for easier searching in the dropdown
...
// Filter customers based on the user's search query
...
// Navigation Logic: Go to the selected customer's info page
function viewAccountDetails() {
  if (!selectedCustomer.value) {
    alert("Please select a customer first.");
    return;
  }
  // Navigate to the InfoPage using the customer's ID as a parameter
...
<template>
  <div class="min-h-screen bg-slate-50 py-12 px-4 flex flex-col items-center space-y-8 font-sans">
    ...
    <div class="w-full max-w-md">
      <label class="block text-lg font-medium text-gray-700 mb-2 text-center">Find Customer</label>
